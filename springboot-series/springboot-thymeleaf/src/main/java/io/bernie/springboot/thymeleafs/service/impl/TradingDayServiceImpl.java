...
public class TradingDayServiceImpl implements TradingDayService {
    ...
    public List<LocalDate> queryBetween(LocalDate from, LocalDate to) {
        ...
        while (leftIndex <= tradingDayList.size() - 1) {
            if (from.compareTo(LocalDate.parse(tradingDayList.get(leftIndex))) <= 0 && to.compareTo(LocalDate.parse(tradingDayList.get(leftIndex))) >= 0) {
                result.add(LocalDate.parse(tradingDayList.get(leftIndex)));
            }
            leftIndex ++;
        }
    ...
    public Optional<LocalDate> queryFirstTradingDayOfMonth(LocalDate date) {
        ...
        Optional<String> temp = tradingDayList.stream().filter(s -> LocalDate.parse(s).getMonthValue() == date.getMonthValue()).findFirst();
        return temp.map(LocalDate::parse);
